...
CNxFPSSprite::CNxFPSSprite(CNxWindow* pParent) : CNxLayerSprite(pParent)
{
	Create(180, 32);		// サイズ適当にサーフェスを作成
	SetZPos(INT_MAX);		// 最上位

	m_font = CNxFont(_T("Arial"), -20);
	SetFont(&m_font);
...
void CNxFPSSprite::PreUpdate()
{
	...
	if (nFPS < 10000 * 1000)
		...
		_stprintf(szBuf, _T("%d FPS"), nFPS / 1000);

	DrawText(2, 2, NULL, szBuf, CNxColor(0, 0, 0, 128));
	DrawText(0, 0, NULL, szBuf, CNxHLSColor(180, 200, 250));
